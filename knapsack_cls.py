...
class KnapsackProblem():
    ...
    * self == other and hash(self) == hash(other) may give different results. __eq__ only compares the value and 
    ...
    @staticmethod
    def _validate_knapsack_parameters(knapsack_items: list[KnapsackItem], knapsack_capacity: int, standing_value: int, standing_knapsack_items: list[KnapsackItem], master_knapsack: KnapsackProblem | None) -> None:
    ...
    def _non_dominated_optimal_terminal_nodes(self) -> list[int]:
        ...
        Mathematically, in these nodes, there are no-dominated items included unless the item was already included at this node.
        """
        return [terminal_node for terminal_node in self._optimal_terminal_nodes if not KnapsackProblem.problems_by_hash[terminal_node]._is_dominated or all(terminal_included_dominated_item in self._included_dominated_items for terminal_included_dominated_item in KnapsackProblem.problems_by_hash[terminal_node]._included_dominated_items)]
    ...
    def print_node_distribution(self, distribution: dict[int, float], parameters: tuple[float, float, float, float] | None = None, print_threshold: float = 0.0001) -> None:
        ...
        for knapsack_problem_hash, distribution_percentage in distribution.items():
            if distribution_percentage > print_threshold:
                ...
                item_inclusion_string = get_item_inclusion_string(knapsack_problem.knapsack_items_repr_hashes, self.knapsack_items_repr_hashes)
                
                if knapsack_problem._master_knapsack is None:  # This is mainly for type hint errors
    ...
    def solve_decision_variant(self, param_alpha: float, param_beta: float, param_gamma: float, param_delta: float, value_threshold: int) -> float:
        """
        Return the expectation (as a percent) of the the time a witness will be found in the knapsack decision problem for the given parameters and value threshold (target).
        ...
        percent_witness_found = sum([distribution_percentage for knapsack_problem_hash, distribution_percentage in node_distribution.items() if KnapsackProblem.problems_by_hash[knapsack_problem_hash]._standing_value >= value_threshold])
